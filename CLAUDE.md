...
- `src/rsl/compile.ts` is the runtime (spec §9): one Subject per state, `observeOn(queueScheduler)` on every inbox, errors resolved per token, completion by an alive-token counter. Two phases: `planMachine` (once per `compile`: validation, registry resolution, nested plans) and `runMachine` (once per subscription: counters, inboxes, reporting; a nested machine is run through it per token). Implemented: Pass, Task, Choice, Succeed, Fail, all four shaping policies, `OnError`. Wait, Parallel and Map throw `RslError` at compile time until their slices land. Helpers: `paths.ts` (JSONPath subset), `evaluate.ts` (data tests, registry resolution), `errors.ts` (error classes, `aslErrorName` / `matchesError` / `errorOutput`).
