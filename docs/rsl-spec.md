...
- **Error names** (`ErrorEquals`): match `error.name`; `States.ALL` matches everything, `States.Timeout` matches RxJS `TimeoutError`. `States.Runtime` is a per-token failure of a state's own fields, such as a Wait whose `SecondsPath` or `TimestampPath` does not hold a duration or a time; Wait has no Catch, so it goes to `OnError`. A Catcher's error token is `{ Error, Cause }` with `Error` the name as `ErrorEquals` sees it (`States.Timeout` for a `TimeoutError`, otherwise `error.name`) and `Cause` the error's message.
...
2. **Wait + cycles** (done): `queueScheduler` trampolining, the feedback-token pitfall. Tests: the polling example, and the loop wrapped in an `exhaust` Task (§8). A Wait is a Task whose resource is the timer, so it shares the Task's accounting and reports `cancel` at teardown; a constant `Timestamp` is checked at compile time, a path that does not hold a duration or a time is a per-token `States.Runtime` error, and a time in the past waits 0 ms.
